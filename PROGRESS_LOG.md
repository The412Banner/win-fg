...
- Env note: device benchmark installed Termux pkgs (libncnn/protobuf/abseil/onnxruntime/…), killed a runaway `pip install ncnn`; artifacts left in the device's tmp dir (benign, note for cleanup). ncnn gets a real Vulkan device only via the root bridge (Adreno750, gpu_count=1), NOT the PRoot sandbox (gpu_count=0).
...
- Checkpoint saved: `winfg-kaggle/checkpoints/winfg_ifnetlite_v5_ep28_psnr26.88.pt` (local training tree) (1.67MB fp32, sha `a0df5f34…`). best.pt/last.pt also on the kernel Output tab.
